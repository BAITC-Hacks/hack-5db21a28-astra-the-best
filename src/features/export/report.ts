...
export function prepareReport({ scenario, simulation, analysis }: ReportExportInput) {
  ...
  return {
    ...
    decisions: verified.decisions.map((decision) => {
      ...
      return { ...decision, name: measure.name, district: decision.districtId ? scenario.districts.find((item) => item.id === decision.districtId)!.name : 'Весь город', cost: measure.cost, lagQuarters: measure.lagQuarters };
    }),
...
export function buildReportHtml(input: ReportExportInput): string {
  ...
  const decisions = report.decisions.map((item, index) => `<tr><td>${index + 1}</td><th scope="row">${escapeHtml(item.name)}</th><td>${escapeHtml(item.district)}</td><td>${item.cost}</td><td>${item.lagQuarters} кв.</td></tr>`).join('');
...
<section><h2>Astana Quality of Life Score</h2><p class="score">${number(result.result.score)}</p><p>До: ${number(result.baseline.score)} · изменение: ${number(result.scoreDelta)}</p><p class="source">Точное значение Score без округления: ${result.result.score}</p><div class="stats"><p>Потрачено <strong>${result.cost} / ${scenario.budget}</strong></p><p>Осталось <strong>${result.remainingBudget}</strong></p><p>Критических показателей <strong>${result.result.criticalCount}</strong></p></div><p>Слабейший район: ${escapeHtml(result.result.weakestDistrictIds.map(districtName).join(', '))} (${number(result.result.minimumDistrictScore)}).</p><p class="source">Score = ${scenario.rules.averageWeight} × средневзвешенный балл + ${scenario.rules.minimumWeight} × балл слабейшего района − ${scenario.rules.criticalPenalty} × число показателей ниже ${scenario.rules.criticalThreshold}.</p></section>

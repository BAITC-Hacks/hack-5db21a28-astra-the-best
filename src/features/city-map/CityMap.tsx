...
import type { DistrictId, IndicatorId, ScenarioResponse } from '@/contracts';
...
export function CityMap({ scenario, selectedDistrictId, onDistrictSelect, onMapReady }: CityMapProps) {
  const container = useRef<HTMLDivElement>(null);
  const mapRef = useRef<MapLibreMap | null>(null);
  const callbacks = useRef({ onDistrictSelect, onMapReady });
  callbacks.current = { onDistrictSelect, onMapReady };
  ...
  useEffect(() => {
    ...
    const values = Object.fromEntries(scenario.districts.map((district) => [district.id, district.indicators[indicatorId]])) as Record<ZoneId, number>;
  ...
  return <section className={styles.shell} aria-label="3D-карта Астаны">
    ...
    <div className={styles.title}><span className={styles.kicker}>ГОРОДСКАЯ СИМУЛЯЦИЯ</span><h1>Астана — ваш город</h1><p>Вся территория города на карте. Выберите игровую зону, приблизьте здания и изучайте город.</p></div>
    ...
    <div className={styles.legend} aria-label="Игровые зоны"><strong>Игровые зоны</strong><label className={styles.layerLabel} htmlFor="indicator-layer">Слой показателей</label><select id="indicator-layer" value={indicatorId ?? ''} onChange={(event) => setIndicatorId(event.target.value ? event.target.value as IndicatorId : null)}><option value="">Обзор зон</option>{scenario.indicators.map((indicator) => <option key={indicator.id} value={indicator.id}>{indicator.name}</option>)}</select>{ZONES.map((zone) => <button key={zone.id} type="button" aria-pressed={selected === zone.id} onClick={() => selectDistrict(zone.id)}><span style={{ background: zone.color }} />{zone.name}{indicatorId && <strong>{scenario.districts.find((district) => district.id === zone.id)?.indicators[indicatorId]}</strong>}</button>)}{activeIndicator && <p className={styles.scale}>Ниже 40 — критично · выше 70 — хорошо. Значения в баллах.</p>}<small>Границы адаптированы для модели из 5 районов. Это не официальное деление.</small></div>
    {selectedZone && selectedDistrict && <div className={styles.zoneCard}><span>ВЫБРАНА ЗОНА</span><h2>{selectedZone.name}</h2><p>{selectedDistrict.profile}</p><dl>{scenario.indicators.map((indicator) => <div key={indicator.id}><dt>{indicator.name}</dt><dd className={selectedDistrict.indicators[indicator.id] < scenario.rules.criticalThreshold ? styles.critical : undefined}>{selectedDistrict.indicators[indicator.id]} / 100</dd></div>)}</dl><button type="button" onClick={() => selectDistrict(null)}>Закрыть</button></div>}

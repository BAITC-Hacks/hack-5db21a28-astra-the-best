...
const coloredScore = (value: number, label = number(value)) => `<span class="score-value" style="color:${scoreColor(value, 'text')}">${label}</span>`;
const isAbsoluteMetricFactId = (id: string) => id === 'score-before' || id === 'score-after' || (id.startsWith('district-') && !id.startsWith('district-change-')) || id.startsWith('before-') || id.startsWith('after-');
...
export function buildReportHtml(input: ReportExportInput): string {
  ...
  const districts = result.result.districts.map((district) => {
    ...
    return `<tr><th scope="row">${escapeHtml(districtName(district.districtId))}</th><td class="numeric">${coloredScore(before.score)}</td><td class="numeric">${coloredScore(district.score)}</td><td class="numeric">${signed(district.score - before.score)}</td></tr>`;
  }).join('');
  ...
  const statement = (item: NonNullable<typeof ai>['analysis']['summary']) => `<p>${escapeHtml(item.text)}</p><p class="source">Основание: ${item.factIds.map((id) => {
    ...
    const value = typeof fact.value === 'number' && isAbsoluteMetricFactId(fact.id)
      ? coloredScore(fact.value)
      : escapeHtml(typeof fact.value === 'number' ? number(fact.value) : fact.value);
    return `${escapeHtml(fact.label)} — ${value} ${escapeHtml(fact.unit)}`;
  }).join('; ')}</p>`;
...
<section><h2>Индекс качества жизни · Score</h2><p class="score">${coloredScore(result.result.score)}</p><p>До решений: ${coloredScore(result.baseline.score)} балла. Изменение: ${signed(result.scoreDelta)} балла.</p><p class="source">Точное значение Score без округления: ${coloredScore(result.result.score, String(result.result.score))}</p><div class="stats"><p>Потрачено <strong>${formatBudget(result.cost)} / ${formatBudget(scenario.budget)}</strong></p><p>Осталось <strong>${formatBudget(result.remainingBudget)}</strong></p><p>Критических показателей <strong>${result.result.criticalCount}</strong></p></div><p>Средний балл города с учётом веса районов: ${coloredScore(result.result.weightedAverage)} балла.</p><p>Слабейший район: ${escapeHtml(result.result.weakestDistrictIds.map(districtName).join(', '))} — ${coloredScore(result.result.minimumDistrictScore)} балла.</p><p>Как рассчитан индекс: ${number(scenario.rules.averageWeight * 100)}% среднего балла с учётом веса районов + ${number(scenario.rules.minimumWeight * 100)}% балла слабейшего района − ${scenario.rules.criticalPenalty} балл за каждый показатель ниже ${scenario.rules.criticalThreshold}. Чем выше индекс, тем лучше итог.</p></section>

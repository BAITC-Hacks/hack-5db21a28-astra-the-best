...
Источник типов: `src/contracts/index.ts` (единственная точка импорта `@/contracts`). Запросы simulate и analyze одинаковы; структурная схема: `src/contracts/scenario-request.schema.json`. Все объекты запросов строгие; дополнительные поля запрещены. Максимум тела 16 KiB. Только JSON, числа конечные. Непредусмотренные поля ответа не добавлять без согласования.
...
Текст русскоязычный, без числовых литералов; значения, меры и показатели привязываются через factIds. UI выводит связанные `facts` рядом с текстом: label, value, unit. Сервер проверяет структуру, существование ссылок и отсутствие числовых литералов в тексте; это не автоматическое доказательство истинности всех слов модели, смысл проверяется при приёмке реального вызова. Нельзя рендерить model text как HTML.

`AnalysisInput` — внутренний серверный контракт: scenario, selectedMeasures (цены, лаги, эффекты), facts, disclaimer. Включает стоимость/остаток, Score до/после и дельту, районные баллы, все изменения показателей, критические значения, минимум и синергии. Facts формируются сервером. Результат AI никогда не перезаписывает числовые поля SimulationResponse.

Рекомендованный UI lifecycle: idle → loading(scenarioId) → success(response) / error(scenarioId,error). Принимать ответ только для текущего scenarioId. Переключение режима город/отчёт не повторяет вызов. Ошибка и retry AI не сбрасывают математический результат. Сервер использует timeout 30 секунд, ограничение повторных вызовов и размера ответа; отсутствие ключа — явная ошибка, без fake-анализа.

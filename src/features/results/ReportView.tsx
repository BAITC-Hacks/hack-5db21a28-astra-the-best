...
function FactRefs({ ids, facts }: { ids: readonly string[]; facts: Map<string, { label: string; value: string | number; unit: string }> }) { const linked = ids.map((id) => facts.get(id)).filter((fact) => fact !== undefined); return linked.length ? <small className={styles.factRefs}>Основание: {linked.map((fact) => `${fact.label} — ${fact.value} ${fact.unit}`).join('; ')}</small> : null; }
